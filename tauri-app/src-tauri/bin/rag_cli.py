...
INDEX_PATH = Path("index") / "faiss_all.index"
META_PATH = Path("index") / "meta_all.jsonl"
EVIDENCE_FOR_LLM = 30
...
def need_disambiguate(anchors: List[str], evidences: List[Dict]) -> bool:
    ...
    return False
def disambiguate_anchors_with_deepseek(question: str, anchors: List[str], candidates_map: Dict[str, List[str]]) -> Dict[str, str]:
    """
    ...
    try:
        r = _call_deepseek_json(system, user, timeout=25, call_name="disambiguate")
    except Exception:
        return {}
...
# Deepseek helper
# ============================================================
def _call_deepseek_json(system: str, user: str, timeout: int = 30, call_name: str = "deepseek_json") -> Optional[Dict]:
...
def classify_question(question: str) -> Dict:
    """
    return e.g.
      {"type": "fact|howto|why|overview", "version_sensitive": true|false}
    """
    api_key = os.getenv("LLM_API_KEY")
    if not api_key:
    ...
    system = (
        ...
        '输出格式：{"intent":"lookup|guide","version_sensitive":true|false}\n'
        "intent含义：\n"
    ...
    user = f"问题：{question}"
    try:
        r = _call_deepseek_json(system, user, timeout=20, call_name="classify")
    except Exception:
        r = None
    if not isinstance(r, dict):
        return {"type": "fact", "version_sensitive": True}
...
def extract_query_plan(question: str, mode: str) -> Dict:
    """
    Deepseek 输出更像“信息抽取结果”，而不是胡乱扩写一大串噪声词。
    return:
    {
      "anchors": ["穿刺", "突进", "力量", "三叉戟", "弓"],
      "rewrite_query": "穿刺 突进 力量 附魔书 适用物品 互斥",
      "subquestions": ["穿刺适用物品与互斥", ...],
    ...
    user = f"问题类型：{mode}\n用户问题：{question}"
    try:
        r = _call_deepseek_json(system, user, timeout=25, call_name="extract_plan")
    except Exception:
        r = None
    if not isinstance(r, dict):
...
def build_messages(
    ...
    rules = [
        "你是一个严格基于证据回答的 Minecraft Wiki助手。",
...
def call_llm(messages: List[Dict]) -> str:
    ...
    if not api_key:
        raise RuntimeError("缺少 LLM_API_KEY 环境变量（请在系统环境变量或当前终端里设置）")
    url = f"{api_base.rstrip('/')}/v1/chat/completions"
    ...
    #  统计 token（粗估）——把 messages 展开为 system+user 字符数
...
def pipeline(
    ...
    *,
    ...
    _progress("正在生成检索计划…")
    qp = extract_query_plan(q, mode=mode)
    ...
    if need_disambiguate(qp.get("anchors", []), evidences):
        ...
        selections = disambiguate_anchors_with_deepseek(q, anchors, candidates_map)
...
def main(argv=None):
    ...
    # 如果你想保持原来 “--question / interactive” 的行为
    ...
    while True:
        ...
        try:
            ...
            continue
        # 如果你想打印消歧结果（可选）
        # dis = ((result.get("debug") or {}).get("disambiguation") or {})
        # if dis.get("triggered"):
        #     print("\n=== 消歧选择 ===")
        #     for a, t in (dis.get("selections") or {}).items():
        #         if t:
        #             print(f"- {a} -> {t}")
